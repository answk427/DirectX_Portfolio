...
{
public:
	//스크린좌표 픽셀 하나를 시야 공간으로 변환하는 함수
	static void ScreenPosToView(float sx, float sy, float* destX, float* destY);
	...
	//시야공간 반직선 -> 세계공간 반직선 변환 
	static std::pair<XMVECTOR, XMVECTOR> ViewToWorldRay(XMVECTOR* rayOrigin, XMVECTOR* rayDir, XMMATRIX* view);
